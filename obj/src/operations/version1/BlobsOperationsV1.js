...
class BlobsOperationsV1 extends pip_services3_facade_node_1.FacadeOperations {
    constructor() {
        super();
        this._dependencyResolver.put('blobs', new pip_services3_commons_node_1.Descriptor('pip-services-blobs', 'client', '*', '*', '1.0'));
    }
            ...
            (callback) => {
                ...
                this._blobsClient.getBlobById(null, blobId, (err, data) => {
                    if (err == null && data == null) {
                        err = new pip_services3_commons_node_2.NotFoundException(null, 'BLOB_NOT_FOUND', 'Blob ' + blobId + ' was not found').withDetails('blob_id', blobId);
                    }
                    blob = data;
                    callback(err);
                });
    ...
    setBlob(req, res) {
        ...
        let expireTime = pip_services3_commons_node_4.DateTimeConverter.toNullableDateTime(req.param('expire_time'));
        let completed = pip_services3_commons_node_3.BooleanConverter.toBoolean(req.param('completed'));
        let blob;
